...
class TablePosition extends Immutable.Record(DEFAULTS) {
   ...
   */
  static create(value, cell, opts) {
    if (cell.type !== opts.typeCell)
      cell = value.document.getClosest(cell.key, p => p.type === opts.typeCell);

    const row = value.document.getClosest(cell.key, p => p.type === opts.typeRow);
    const table = value.document.getClosest(cell.key, p => p.type === opts.typeTable);
